...
class MyMemCell(tf.nn.rnn_cell.RNNCell):
    ...
    def __init__(self, num_units, memory, cell, config):
        ...
        self.Wp = tf.get_variable('Wp', shape=(self.config.decoder_hidden_size,\
                                self.config.decoder_hidden_size), \
                                initializer=tf.contrib.layers.xavier_initializer())
        self.Wi = tf.get_variable('Wi', shape=(self.config.embedding_dim,\
                                self.config.decoder_hidden_size), \
                                initializer=tf.contrib.layers.xavier_initializer())
    ...
    def __call__(self, inputs, state, scope=None):
        ...
        new_inputs = tf.tanh(tf.matmul(inputs, self.Wi) + tf.matmul(r_t, self.Wp))
