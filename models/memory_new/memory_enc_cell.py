...
class MyMemEncCell(tf.nn.rnn_cell.RNNCell):
    ...
    def __init__(self, cell, config):
        ...
        self.cell = cell
        self.config = config
        self.Wc = tf.get_variable('WcAtt', shape=(2*self.config.encoder_hidden_size + memory.get_shape().as_list()[-1],\
                                self.config.encoder_hidden_size), \
                                initializer=tf.contrib.layers.xavier_initializer())
        self.W_ers = tf.get_variable('WErs', shape=(self.config.encoder_hidden_size,\
                                self.config.encoder_hidden_size), \
        ...
        self.w_rg = tf.get_variable('w_rg', shape=(self.config.encoder_hidden_size, 1), \
                                initializer=tf.contrib.layers.xavier_initializer())
    ...
    def state_size(self):
        ...
        sizes = [self.config.encoder_hidden_size, self.config.num_cells, self.config.num_cells]
