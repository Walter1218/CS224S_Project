...
class ASRModel:
	...
	def add_encoder(self):
		...
		with tf.variable_scope('Encoder'):
			...
			self.encoded = tf.reshape(self.encoded, [config.batch_size, 2*config.encoder_hidden_size])	
	...
	def add_cell(self):
		with tf.variable_scope('MyDecoderCell'):
			...
			self.decoder_cell = tf.contrib.seq2seq.AttentionWrapper(cell=cell, attention_mechanism=att_mech, \
									attention_layer_size=config.decoder_hidden_size, alignment_history=False)

			# We need to convert the final hidden state of the encoder into something that is compatible
			attn_zero = self.decoder_cell.zero_state(batch_size=config.batch_size, dtype=tf.float32)
		...
		with tf.variable_scope('MyDecoderCell', reuse=True):
			...
			cell = tf.contrib.rnn.GRUCell(num_units = config.decoder_hidden_size)
			print 'Cell', cell
			tiled_memory = tf.contrib.seq2seq.tile_batch(self.memory, multiplier=config.num_beams)
			tiled_init_state = tf.contrib.seq2seq.tile_batch(self.encoded, multiplier=config.num_beams) 
			tiled_seq_lens = tf.contrib.seq2seq.tile_batch(self.input_seq_lens, multiplier=config.num_beams)

			# We will use Luong Attention
			att_mech = tf.contrib.seq2seq.LuongAttention(num_units=config.decoder_hidden_size, memory=tiled_memory, \
			...
			# We need to convert the final hidden state of the encoder into something that is compatible
			attn_zero = self.decoder_cell.zero_state(batch_size=config.num_beams*config.batch_size, dtype=tf.float32)
	...
	def add_decoder_test(self):
		...
		with tf.variable_scope(scope, reuse=True):
			...
			# We need to convert the final hidden state of the encoder into something that is compatible
			attn_zero = self.beam_decoder_cell.zero_state(batch_size=config.num_beams*config.batch_size, dtype=tf.float32)
			
			self.init_beam_state = attn_zero.clone(cell_state=tiled_init_state)

			print 'Init beam state', self.init_beam_state
			first_tokens = self.labels_placeholder[:, 0]
			tiled_first_tokens = tf.contrib.seq2seq.tile_batch(first_tokens, multiplier=config.num_beams)
			...
			beam_search_decoder = tf.contrib.seq2seq.BeamSearchDecoder(cell=self.beam_decoder_cell, embedding=self.L,\
													...
													beam_width=config.num_beams, output_layer=self.output_layer, length_penalty_weight=0.0)

			final_outputs, final_state, final_sequence_lengths = tf.contrib.seq2seq.dynamic_decode(beam_search_decoder, \
									output_time_major=True, maximum_iterations=config.max_out_len)
			...
			# attn_zero = self.decoder_cell.zero_state(batch_size=tf.shape(self.encoded)[0]*config.num_beams, \
			# 			dtype=tf.float32)
