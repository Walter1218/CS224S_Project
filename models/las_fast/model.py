...
class ASRModel:
	...
	def concatenate(self, inputs, sequence_lengths, tot_len):
		# concat_inputs = []
		# for time_i in range(1, tot_len, 2):
		...
		first = inputs[:, ::2]
		second = inputs[:, 1::2]
		new_inputs = tf.concat(2, (first, second))
		print('concat: concat shape: ', tf.Tensor.get_shape(new_inputs))

		sequence_lengths = tf.cast(tf.floor(tf.cast(sequence_lengths, tf.float32)/2), tf.int32)
		return new_inputs, sequence_lengths
	...
	def add_encoder(self):
		...
		with tf.variable_scope('Encoder'):
			...
			outputs, states = tf.nn.bidirectional_dynamic_rnn(cell_fw = cell_fw, cell_bw = cell_bw, sequence_length=self.input_seq_lens, inputs=self.input_placeholder, dtype=tf.float32, scope='BLSTM')
			h1_vals = tf.concat(2, outputs)

			# First PLSTM
			print 'First PLSTM'
			concat_inputs, new_seq_lens = self.concatenate(h1_vals, self.input_seq_lens, self.config.max_in_len)
			...
			outputs2, states2 = tf.nn.bidirectional_dynamic_rnn(cell_fw = cell_fw2, cell_bw = cell_bw2, sequence_length=new_seq_lens, inputs=concat_inputs, dtype=tf.float32, scope='PLSTM1')
			...
			# Second PLSTM
			print 'Second PLSTM'
			concat_inputs2, new_seq_lens2 = self.concatenate(h2_vals, new_seq_lens, int(self.config.max_in_len/2.0))
			...
			outputs3, states3 = tf.nn.bidirectional_dynamic_rnn(cell_fw = cell_fw3, cell_bw = cell_bw3, sequence_length=new_seq_lens2, inputs=concat_inputs2, dtype=tf.float32, scope='PLSTM2')
			...
			# Final PLSTM
			print 'Final PLSTM'
			concat_inputs3, new_seq_lens3 = self.concatenate(h3_vals, new_seq_lens2, int(self.config.max_in_len/4.0))
			...
			final_outputs, final_states = tf.nn.bidirectional_dynamic_rnn(cell_fw = cell_fw3, cell_bw = cell_bw3, sequence_length=new_seq_lens3, inputs=concat_inputs3, dtype=tf.float32, scope='PLSTM3')
			all_states = []
			for i in range(self.config.num_layers):
