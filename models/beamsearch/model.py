...
class ASRModel:
	...
	def add_decoder(self):
		...
		with tf.variable_scope(scope):
			...
			# logits_flat = tf.layers.dense(inputs=outputs_flat, units=config.vocab_size, \
	...
	def add_decoder_test(self):
		...
		with tf.variable_scope(scope, reuse=True):
			...
			init_state = tf.tile(self.encoded, multiples=[config.num_beams, 1])
			...
			beam_search_decoder = tf.contrib.seq2seq.BeamSearchDecoder(cell=cell, embedding=self.L,\
													...
													beam_width=config.num_beams, output_layer=self.output_layer)
